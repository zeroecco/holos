...
//   - A set of [config.WriteFile] entries from COPY/ADD instructions
//     (file sources only; directories are not supported).
//
...
// HEALTHCHECK, STOPSIGNAL, SHELL, ONBUILD, ARG, USER) are silently skipped.
// Multi-stage builds (COPY --from=) are rejected with an error.
package dockerfile
